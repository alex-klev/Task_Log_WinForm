...
class BosesCatalog(QtWidgets.QMainWindow):
    def __init__(self, **kwargs):
        ...
        self.ui_boses_catalog = Ui_BosesCatalogWindow()  # Создаем экземпляр Ui_BosesCatalogWindow
        self.ui_boses_catalog.setupUi(self)  # Загружаем интерфейс из файла
        ...
        self.ui_boses_catalog.btnDelete.clicked.connect(self.delete_data_from_db)  # Удалить запись из таблицы
        self.ui_boses_catalog.btnSave.clicked.connect(self.save_data_in_db)  # Сохранить запись в таблице
        self.ui_boses_catalog.btnCancel.clicked.connect(self.cancel)  # Отменить запись из таблицы
        
        self.ui_boses_catalog.tableView.clicked.connect(self.on_cell_clicked)  # Клик по ячейке
        self.ui_boses_catalog.tableView.activated.connect(self.on_cell_activated)  # Данные в выбранной строке таблицы
        # self.ui_boses_catalog.tableView.pressed.connect(self.on_cell_pressed)  # Данные в выбранной строке таблицы
        
        self.ui_boses_catalog.lineEdit.setEnabled(False)
        
        # Создаём модель таблицы
        table_model = TableModel(
            ui=self.ui_boses_catalog, # Передаём ссылку на ui_personal_plan
    ...
    def load_data_db(self):
        try:
            ...
            if not data or data == [('',)]:
                ...
                """
                model = self.ui_boses_catalog.tableView.model()
    ...
    def load_table(self, data):
        ...
        if not data: 
            self.ui_boses_catalog.tableView.setModel(None)
            return
        ...
        # self.ui_boses_catalog.tableView.setEditTriggers(QTableView.NoEditTriggers)  # QTableView наследуется от QAbstractItemView
        self.ui_boses_catalog.tableView.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    def add_data_to_db(self) -> None:
        ...
        self.ui_boses_catalog.lineEdit.setFocus(True)
    ...
    def update_data_in_db(self) -> None:
        ...
        self.ui_boses_catalog.lineEdit.setFocus(True)
        # Выбранная строка
        current_row = self.ui_boses_catalog.tableView.currentIndex().row()
    ...
    def delete_data_from_db(self) -> None:
        ...
        delete = QMessageBox.warning(self,
                                      ...
                                      'Вы хотите удалить запись "{0}" ?'.format(
                                          self.ui_boses_catalog.tableView.model().index(row, 1)),
        ...
        msg_box.setText("Вы уверены, что хотите удалить запись '{0}' ?".format(self.ui_boses_catalog.tableView.model().data(self.ui_boses_catalog.tableView.model().index(current_row, 1))))
        ...
        if result == QtWidgets.QMessageBox.Yes:
            ...
            if current_row == -1:
                ...
                data_id = self.ui_boses_catalog.tableView.model().index(current_row, 0).data().strip()  # data.get('data_id') # id записи, присвоенная в таблице на стороне клиента (Date.now().toString() дата в виде текста)
    ...
    def save_data_in_db(self) -> None:
        ...
        if self.setSql == "Insert":
            ...
            fio = self.ui_boses_catalog.lineEdit.text().strip()  # Ф.И.О.
            ...
            if not fio or fio.strip() == "":
                ...
                bosses_catalog.double_data()
                
                if bosses_catalog.double_data() != []:
                    QMessageBox.warning(self, 'Внимание', 'Запись руководителя "{0}" уже существует'.format(fio))
                    return
    ...
    def bool_enabled_elements(self, boolean) -> None:
        ...
        self.ui_boses_catalog.btnSave.setEnabled(not boolean)
        self.ui_boses_catalog.btnCancel.setEnabled(not boolean)
        self.ui_boses_catalog.btnInsert.setEnabled(boolean)
        self.ui_boses_catalog.btnUpdate.setEnabled(boolean)
        self.ui_boses_catalog.btnDelete.setEnabled(boolean)
        
        self.ui_boses_catalog.lineEdit.setEnabled(not boolean)
        # self.ui_boses_catalog.lineEditName.setEnabled(not boolean)
        # self.ui_boses_catalog.lineEditPatronymic.setEnabled(not boolean)
        
        self.ui_boses_catalog.lineEditFilter.setEnabled(boolean)
        self.ui_boses_catalog.lineEditFilter.setText("")
        
        self.ui_boses_catalog.tableView.setEnabled(boolean)
    
    def clear_line_edit(self) -> None:
        """Очистка полей"""
        self.ui_boses_catalog.lineEdit.setText("")
        # self.ui_boses_catalog.spinBoxGoalEvaluation.setValue(0)
        # self.ui_boses_catalog.spinBoxAgeGoal.setValue(0)
